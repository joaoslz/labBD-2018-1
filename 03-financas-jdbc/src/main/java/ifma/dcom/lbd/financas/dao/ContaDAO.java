package ifma.dcom.lbd.financas.dao;
...
public class ContaDAO {
	...
	public void altera(Conta conta) {
		...
		try {
			PreparedStatement ps = this.connection
					.prepareStatement("update Conta set titular=?, banco=?, agencia=?, numero=? "
							           + "where id=?");
			ps.setString(1, conta.getTitular());
			ps.setString(2, conta.getBanco());
			ps.setString(3, conta.getAgencia());
	...
	public Conta procuraPelo(Integer id) {
		...
		try {
			PreparedStatement ps = this.connection
					.prepareStatement("select titular, banco, numero, agencia "
							+ "from Conta "
							+ "where id = ?");
			ps.setInt(1, id);
	...
	public List<Conta> procuraPeloTitular(String nome) {

		String sql = "select titular, banco, numero, agencia "
				+ "from Conta "
				+ "where titular like ?";
